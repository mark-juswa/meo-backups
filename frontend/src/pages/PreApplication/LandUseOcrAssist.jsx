...
const Field = ({ field, value, onChange, status }) => {
      ...
      >
        <label className="flex items-center text-sm font-medium text-gray-700">
          <input
            ...
            onChange={(e) => onChange({ target: { name: field.key, value: e.target.checked ? 'checked' : '' } })}
            className="mr-2 h-4 w-4 text-blue-600 rounded"
          />
          {field.label}
        </label>
        {isDetected && <p className="text-xs text-amber-700 mt-1">Extracted (editable)</p>}
        {isMissing && <p className="text-xs text-red-700 mt-1">Not detected</p>}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-5xl mx-auto p-4 sm:p-8">
        <div className="bg-white rounded-xl shadow border p-4 sm:p-8">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Land Use / Zoning Clearance</h1>
          <p className="text-sm text-gray-600 mt-2">
            Upload a scanned Land Use / Zoning / Locational Clearance document. The form only reflects what was extracted so you can verify and correct it.
          </p>
          ...
          {step === 'review' && (
            <div className="mt-6">
              ...
              <div className="mt-4">
                ...
                {['Administrative', 'Applicant Details', 'Project Details', 'Land Rights', 'Project Areas', 'Land Use', 'Administrative Process', 'Signatures'].map((section) => {
                  const sectionFields = LAND_USE_FIELDS.filter(f => f.section === section);
